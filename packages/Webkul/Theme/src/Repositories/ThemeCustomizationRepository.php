    ...
    public function uploadImage(array $data, ThemeCustomization $theme)
    {
        ...
        foreach ($data[$locale]['options'] as $image) {
            if (isset($image['service_icon'])) {
                ...
                try {
                    $extension = strtolower($image['image']->getClientOriginalExtension());

                    if (in_array($extension, ['gif', 'webm'])) {
                        $path = $image['image']->storeAs('theme/'.$theme->id, Str::random(40).'.'.$extension);
                    } else {
